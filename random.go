...
// getSecureByteSlice produces a slice of random bytes, of a provided length
// used as the base for other outputs
func getSecureByteSlice(length int) (bs []byte, err error) {
	bs = make([]byte, length)
	r, err := rand.Read(bs)
	if err != nil {
		return nil, fmt.Errorf("securerandom/getSecureByteSlice: failed to create secure slice of bytes \n%v", err)
	}
	if r != length {
		return nil, fmt.Errorf("securerandom/getSecureByteSlice: failed to create %v byte string", length)
	}
	return bs, nil
}
